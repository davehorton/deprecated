...
,SipDialog = require('drachtio-dialog').SipDialog
...
Msml.prototype.makeConnection = function( ms, opts, callback ) {
    ...
    if( !callback ) {
        ...
        request.pipe = function( res, opts, cb ) {
            ...
            pipe.call( request, res, opts, function( uacRes ){
                if( uacRes.statusCode === 200 ) {
                    debug('got 200 OK from ms in makeConnection')
                    var req = uacRes.req ;
                    var dialog = new SipDialog( req ) ;
                    dialog.setConnectTime( uacRes.time );
