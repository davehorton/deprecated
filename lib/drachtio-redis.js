var debug = require('debug')('drachtio:redis');
...
/**
 * Return the `RedisStore` extending `drachtio`'s session Store.
 *
 * @param {object} drachtio
 * @return {Function}
 * @api public
 */

module.exports = function(drachtio){

  /**
   * Connect's Store.
   */

  var Store = drachtio.session.Store;
  ...
  RedisStore.prototype.set = function(sid, sess, callback){
    var self = this ;
    (function(sid, sess, fn){
      sid = self.prefix + sid;
      try {
        var ttl = self.ttl
