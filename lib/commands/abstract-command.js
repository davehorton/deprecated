		...
		if( child.name in opts ) {
			...
			if( _.isArray( data ) ) {
				...
				data.forEach( function( achild ){
					el.push( new child.Klass( achild ) ) ;
				});
				self.children.push( el ) ;
			}
			else self.children.push( new child.Klass( data ) );
		}
	}) ;
}
...
AbstractCommand.prototype.appendChild = function( child ) {
...
AbstractCommand.prototype.toXMLNode = function() {
	...
	debug('creating xml with %d nodes', this.children.length) ;
	
	this.children.forEach( function( child ){
		if( _.isArray( child ) ) {
			child.forEach( function( achild ){
				root.append( achild.toXMLNode() ) ;
			}) ;
		}
		else {
			root.append( child.toXMLNode() ) ;
		}
