    ...
    var request = this.siprequest( msUrl, {
      ...
      if( res.statusCode === 200 ) {
            res.ack(function(err, sipDialog) {
                ...
                /* if we are given a session object, add the channel as a key */
                var mks ;
                if( opts.session ) {
                    mks = opts.session.mks ;
                    opts.session.mks.set( channel ) ;
                    Object.defineProperty( channel, 'session', opts.session ) ;
                }

                var req = res.req ;
                var e = new Event( channel, mks ) ;
                e.req = req ;
                e.res = res ;
                ...
                if( callback ) {
                    process.nextTick( function() {
                        /* in case caller modifies session in the callback */
                        mks.save() ;
                    }) ;
